...
WITH all_booking_transaction AS (
    ...
    FROM {{ ref('raw_booking__raw_flight_booking') }} fb
    JOIN {{ ref('raw_products__raw_product_catalog') }} pc ON fb.product_id = pc.product_id
    UNION ALL
    SELECT
        hb.customer_id,
    ...
    FROM {{ ref('raw_booking__raw_hotel_booking') }} hb
    JOIN {{ ref('raw_products__raw_product_catalog') }} pc ON hb.product_id = pc.product_id
    UNION ALL
    SELECT
        crb.customer_id,
    ...
    FROM {{ ref('raw_booking__raw_car_rental_booking') }} crb
    JOIN {{ ref('raw_products__raw_product_catalog') }} pc ON crb.product_id = pc.product_id
    UNION ALL
    SELECT
        ab.customer_id,
    ...
    FROM {{ ref('raw_booking__raw_attraction_booking') }} ab
    JOIN {{ ref('raw_products__raw_product_catalog') }} pc ON ab.product_id = pc.product_id
    UNION ALL
    SELECT
        bb.customer_id,
    ...
    FROM {{ ref('raw_booking__raw_bundle_booking') }} bb
    JOIN {{ ref('raw_products__raw_product_catalog') }} pc ON bb.primary_product_id = pc.product_id
)
SELECT
